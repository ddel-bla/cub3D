...
void render_frame(t_game *game)
{
    ...
    while (x < WIN_W)
    {
        ...
        double ray_dir_x = game->pla.dir_x + game->pla.plane_x * camera_x;
        double ray_dir_y = game->pla.dir_y + game->pla.plane_y * camera_x;

        // Posición del jugador en la cuadrícula
        int map_x = (int)game->pla.pos_x;
        int map_y = (int)game->pla.pos_y;
        ...
        {
            step_x = -1;
            side_dist_x = (game->pla.pos_x - map_x) * delta_dist_x;
        }
        else
        {
            step_x = 1;
            side_dist_x = (map_x + 1.0 - game->pla.pos_x) * delta_dist_x;
        }
        if (ray_dir_y < 0)
        {
            step_y = -1;
            side_dist_y = (game->pla.pos_y - map_y) * delta_dist_y;
        }
        else
        {
            step_y = 1;
            side_dist_y = (map_y + 1.0 - game->pla.pos_y) * delta_dist_y;
        }
        ...
        if (side == 0)
            perp_wall_dist = (map_x - game->pla.pos_x + (1 - step_x) / 2) / ray_dir_x;
        else
            perp_wall_dist = (map_y - game->pla.pos_y + (1 - step_y) / 2) / ray_dir_y;
        ...
        draw_line(&game->win.img, x, draw_start, draw_end, color);

        x++;
    }

    // Actualizar la ventana con la imagen renderizada
    mlx_put_image_to_window(game->win.mlx_p, game->win.win_p, game->win.img.img_ptr, 0, 0);
}
